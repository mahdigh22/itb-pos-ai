...
export default function DashboardClient() {
  ...
  useEffect(() => {
    ...
    if (adminData) {
      ...
      const q = query(
        collection(db, "restaurants", admin.restaurantId, "orders"),
      ...
      const unsubscribe = onSnapshot(q, (querySnapshot) => {
        ...
        console.log("Order data:", liveOrders);
  ...
  const reportData = useMemo(() => {
    ...
    const totalPrepTime = orders.reduce(
      (acc, order) => acc + (order.totalPreparationTime || 0),
      0
    );
    ...
    const totalProfit = orders.reduce((profitAcc, order) => {
      ...
      const discountAmount =
        orderSubtotal * ((order.discountApplied || 0) / 100);
    ...
    const topSellingItems = orders
      ...
      .filter((item) => item.status !== "edited" && item.status !== "cancelled")
      .reduce((acc, item) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {reportData.averagePrepTime.toFixed(1)} {t("minutes")}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        <Card className="lg:col-span-4">
          <CardHeader>
            ...
            <CardDescription>{t("revenueLast30Days")}</CardDescription>
          </CardHeader>
        ...
        <Card className="lg:col-span-3">
          <CardHeader>
            ...
            <CardDescription>{t("orderTypesBreakdown")}</CardDescription>
          </CardHeader>
